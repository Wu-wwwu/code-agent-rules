...
LOCAL_AGENT_POINTERS = {"mini/agent.md", "rules/agent.md", "self/core.md"}
...
def check_agents_pointer(errors: list[str]) -> None:
    ...
    if not is_absolute:
        if normalized not in LOCAL_AGENT_POINTERS:
            errors.append(
                "AGENTS.md: 相对指针只能是 mini/agent.md、rules/agent.md 或 self/core.md"
            )
            return
    ...
    if not normalized.endswith(AGENT_ENTRY_SUFFIXES):
        errors.append(
            "AGENTS.md: 固定路径必须指向 mini/agent.md、rules/agent.md 或 self/core.md"
        )
        return
