...
> 三层规则体系：基础规范 + 候选路由 + 专项方法。轻量、单系统、渐进展开。

## 设计原则

1. **基础规范**：核心原则与六步执行循环，由平台在每轮请求中纳入上下文
2. **短表路由**：任务开始或范围变化时读取候选信号摘要，不在触发表重复执行细节
3. **方法按需**：详细适用条件、快速路径和执行指导在对应方法中确认
...
mini/
├── README.md              # 本文件
├── agent.md               # L1 基础规范（每轮纳入上下文，含六步循环）
...
**入口文件**：`agent.md`。Agent 从该文件开始，它在头部明确了完整加载链：

```
加载 agent.md
  ↓
观察并校准任务 → 读取 triggers.md 识别候选方法
  ↓ 命中候选信号
加载最少的 methods/ 文件确认详细适用条件
  ↓
按六步循环执行；专项义务完成后退出方法
```
