...
本仓库是一套可被 AI Coding Agent 加载的行为规则。规则正文与具体工具无关；本文件是跨工具通用入口，只负责告诉 Agent「去读哪些文件」，不把全部内容塞进上下文（遵循规则自身的「按需取证」原则）。工具是否自动读取 `AGENTS.md` 取决于平台能力和配置；不自动读取时可在平台实际支持的项目指令位置引用本文件。
...
> 更多说明见仓库 `README.md`。仓库当前另提供 `CLAUDE.md`；其他工具可在其实际支持的配置位置引用本入口。
