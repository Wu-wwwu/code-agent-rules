# Code Agent Rules — 跨工具加载入口（AGENTS.md）

本仓库是一套可被 AI Coding Agent 加载的行为规则。规则正文与具体工具无关；本文件只负责告诉 Agent「去读哪些文件」，不把全部内容塞进上下文（遵循规则自身的「按需取证」原则）。多数现代 Agent（Claude Code / OpenAI Codex / GitHub Copilot / Cursor / Trae）都会自动读取仓库根目录的 `AGENTS.md`。
